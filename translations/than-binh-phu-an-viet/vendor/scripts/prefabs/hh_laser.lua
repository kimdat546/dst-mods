...
local DAMAGE_CANT_TAGS = { "playerghost", "INLIMBO", "DECOR", "INLIMBO" }
...
local function DoDamage(inst, targets, skiptoss)
    ...
    if inst["AnimState"] ~= nil then
        ...
        inst:DoTaskInTime(4 * FRAMES, SetLightRadius, 0.5)
        inst:DoTaskInTime(5 * FRAMES, DisableLight)

        SpawnPrefab("hh_deerclops_laserscorch")["Transform"]:SetPosition(x, 0, z)
        local fx = SpawnPrefab("hh_deerclops_lasertrail")
        ...
        if not targets[v] and v:IsValid() and not v:IsInLimbo() and not (v["components"]["health"] ~= nil and v["components"]["health"]:IsDead()) then
            ...
            if v:GetDistanceSqToPoint(x, y, z) < range * range then
                ...
                then
                    ...
                    if v:IsValid() then
                        SpawnPrefab("hh_deerclops_laserhit"):SetTarget(v)
                    end
                end
            end
        end
...
local function common_fn(isempty)
    ...
    if not isempty then
        ...
        inst["Light"]:SetColour(255 / 255, 242 / 255, 0 / 255)
        inst["Light"]:Enable(false)
    end
    inst:Hide()
    ...
    inst:SetPrefabNameOverride("hh_laser_fx")
...
local function UpdateHit(inst, target)
    if target:IsValid() then
        ...
        inst["flash"] = math.max(0, inst["flash"] - 0.075)
        if inst["flash"] > 0 then
            ...
            if inst["flash"] < 0.3 and oldflash >= .3 then
...
Prefab("hh_deerclops_lasertrail", trailfn, assets_trail),
Prefab("hh_deerclops_laserhit", hitfn)
